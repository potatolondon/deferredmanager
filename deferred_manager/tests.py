...
class DeferTaskTests(BaseTest):
    ...
    def test_unique_task_ref(self):
        ...
        self.assertRaises(
            AssertionError, defer, noop, unique_until=unique_until)
        self.assertTrue(
            defer(noop, task_reference="project1", unique_until=unique_until))
        self.assertFalse(
            defer(noop, task_reference="project1", unique_until=unique_until))
    ...
    def test_class_method_repr(self):
        ...
        self.assertEqual(
            task_state.deferred_function, u"<class 'deferred_manager.tests.Foo'>.bar")
...
class HandlerTests(BaseTest):
    def make_request(
        ...
        return webapp2.Request.blank(
            '/', environ=request_environ, headers=request_headers, **kwargs)
